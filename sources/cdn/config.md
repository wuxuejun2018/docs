...
`TLS 1.3` 是 TLS 协议中最新、最快和最安全的版本，相比旧版的 TLS 协议增加了多项新功能。通过简化 SSL 握手，提高了建连速度，减少了延迟。并通过移除有安全隐患的加密算法，提高了用户访问的性能、效率和安全性等等。目前 `TLS 1.3` 在又拍云 CDN 平台已全网支持，在 CDN 控制台您可以灵活配置。
